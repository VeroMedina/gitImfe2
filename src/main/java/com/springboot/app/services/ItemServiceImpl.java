...
public class ItemServiceImpl implements ItemService {
	...
	/**
	 * METODO QUE RECIBE EL OBJETO BUSCADO Y LA LISTA DE LOS OBJETOS BUSCADOS DE LA
	 * BBDD. SI EL OBJETO BUSCADO COINCIDE CON EL DE LA BBDD SERA REEMPLAZADO SU
	 * ESTILO
	 *
	 * @param obj
	 *            -> objeto,nombre o descripcion buscado.
	 *
	 * @param x
	 *            -> lista de objetos de la bbdd buscados con todos sus propiedades
	 *
	...
	public List<ItemModel> subrayarItemsService (ItemModel obj, List<ItemModel> x) throws Exception
	{
		...
		/**
		 * Recorro el resultado de la bbdd de los items encontrados segun la busqueda en
		 * item va los resutaldos de x.
		 */
		for (ItemModel item : x) 
		{
			...
			/**
			 * Se almacena el dato original de la bbdd y el dato buscado, para su posterior uso en la
			 * sustitucion final.
			 */
			...
			// Conversion a UTF-8(sin acento) de la palabra que se busca
			...
			if (objMatch.find()) 
			{
				 ...
				 */
				item.setNombre(
						UtilStr.pattern(buscado).matcher(originalUTF).replaceAll(UtilStr.patternReplaceHtml(original)));
			...
			// Ejecutamos metodo para comprobar que lo buscado coincide con lo encontrado y
			// si es que si devuelve un true
			Matcher boolDescrip = UtilStr.coinciden(buscadoDescrip, originalDescripUTF);
			if (boolDescrip.find()) 
			{
				System.out.println("boolDescrip.find() ---> ");
				// Meto en la variable 'original' la parte de la palabra de la bbdd que coincide
				// con lo buscado
				originalDescrip = originalDescrip.substring(boolDescrip.start(), boolDescrip.end());

				item.setDescripcion(UtilStr.pattern(buscadoDescrip).matcher(originalDescripUTF)
